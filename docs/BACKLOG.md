...
- [ ] **jsx-runtime duplicado nos remotes** — cada remote serve a sua própria cópia de `react/jsx-runtime` porque este módulo não está no shared config da federation. Duas abordagens a analisar: (1) adicionar `react/jsx-runtime` e `react/jsx-dev-runtime` ao `DEFAULT_SINGLETONS` no `@-label-/federation-config` — garante que o runtime do host é partilhado com os remotes; (2) configurar `@vitejs/plugin-react` com `jsxRuntime: 'classic'` (injecta `React.createElement` em vez de `jsx()`) — se React já é singleton, o createElement vem do mesmo React. Avaliar trade-offs: a opção 1 é mais alinhada com a arquitectura actual (shared deps), a opção 2 simplifica mas perde o automatic runtime. Confirmar com teste E2E que após o fix só existe uma instância do jsx-runtime no browser
